...
bool end_of_runs=false;
//-----------------------
/**
  Design of the singleinds version of the IBC-grass_coast model:

  On an 1.28 by 1.28cm square grid 10 for one of x PFTs
  (parameterized according to real species) are initiated.
  One environmental setting is tested for 10 years (10 repititions).
  Salinity and water levels are constant.
  No block design.
...
//		string PFTname=var->first;
		std::cout<<Envir->SimNr<<endl;
//  		InitClonalSeeds(traits, no_init_seeds); //cltraits,
	     //Open InitFile,
	   	end_of_runs=!SWaterTraits::ReadWPFTDef(SRunPara::NamePftFile,Envir->SimNr++);
//		Envir->SimNr=6;end_of_runs=!SWaterTraits::ReadWPFTDef(SRunPara::NamePftFile,5);//Arrhenatherun
//Envir->SimNr++;
    //Run-loop
...
//-----------------
 ...
 //-----------------
      ...
      cout<<Envir->year
    		  <<"\t"<<Envir->GridOutData.back()->NPlants// PftCount
    		  <<"\t"<<Envir->GridOutData.back()->NSeed// shannon
