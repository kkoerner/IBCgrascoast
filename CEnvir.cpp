...
//	WriteclonalOutput();
	if (false){//(year == SRunPara::RunPara.Tmax){//output at end of run
		WriteSurvival();
   ...
   if (size==0){
     GridOutFile<<"Sim\tRun\tTime\t"
           ...
           // <<"abovemass\tbelowmass\tBareGround\t"
              ...
              <<"NPFT\tCutted\tGrazed\t"
              <<"aLitter\tbLitter"
             <<"NNonClonal\tNClonal\tNGenets\tMSeed\tNSpacer"
              ;
     GridOutFile<<"\n";
   }
...
//                 <<'\t'<<GetMeanNPFT(10)
                 ...
                 <<'\t'<<GridOutData[i]->NPlants
                 <<'\t'<<GridOutData[i]->NclonalPlants
                 <<'\t'<<GridOutData[i]->NGenets//GetNMotherPlants()
                 <<'\t'<<GridOutData[i]->MSeed
                 <<'\t'<<GridOutData[i]->NSpacer
              <<"\n";
   }
   GridOutFile.close();
   //cout debugging information
//   cout<<GridOutData.back()->above_mass<<'\t'<<GridOutData.back()->Nind<<'\t'<<GridOutData.back()->Nind<<endl;
}//WriteGridComplete
//------------------------------------------------------------------------------
/**
